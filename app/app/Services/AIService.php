            ...
            if ($tenant->google_sheet_id && $tenant->google_sheet_sync_mode === 'smart_read_write') {
                ...
                try {
                    ...
                    if (!empty($sheetData) && !empty($sheetData['grid_text'])) {
                        ...
                        $prompt .= "Whenever the customer confirms or updates any key transaction detail (such as item/service/product selected, booking date, quantity, price, any add-on or extra, advance/deposit paid, or their name and phone number), you MUST append a CONTEXT_STATE block at the very end of your response so the system can persist it securely in the database across the conversation:\n";
                        $prompt .= "CONTEXT_STATE: {\"item\": \"<confirmed item, product, room, dome, table, etc>\", \"date\": \"<confirmed date if applicable>\", \"quantity\": \"<confirmed count, guests, units>\", \"price\": \"<confirmed total price>\", \"addon\": \"<any confirmed add-ons or extras>\", \"advance\": \"<deposit or advance paid>\", \"customer_name\": \"<confirmed name>\", \"customer_phone\": \"<confirmed phone>\"}\n";
                        $prompt .= "Use key names that naturally fit this business context (e.g. 'dome' for glamping, 'room' for hotel, 'product' for shop, 'service' for agency, 'property' for real estate). Only include keys that are now confirmed — omit anything still unknown. This block is invisible to the customer; strip it cleanly from your conversational reply.\n\n";
                    }
                } catch (\Throwable $e) {
